...
profissionais_repetidos <- df_limpo |> 
  ...
  select(nome, nome_do_municipio, nivel_atencao, codigo_profissional_saude, codigo_estabelecimento_saude) |> 
  arrange(codigo_profissional_saude)

profissionais_repetidos |> 
  select(nome, nivel_atencao, codigo_profissional_saude, codigo_estabelecimento_saude) 
...
profissionais_repetidos |> 
  group_by(nome, codigo_profissional_saude) |> 
  summarise(vinculos = n(), .groups = "drop") |> 
  arrange(desc(vinculos)) |> 
  print(n = Inf)
...
df_limpo |> 
  select(nome, nivel_atencao, estabelecimento) |> 
  filter(nivel_atencao == "atenção terciária")

# número de estabelecimentos 
df_limpo |> 
  group_by(nivel_atencao) |> 
  summarise(total_estab = n_distinct(codigo_estabelecimento_saude)) |> 
  mutate(perc = total_estab / sum(total_estab) * 100)
