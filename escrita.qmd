...
total_estab <- n_distinct(df_limpo$codigo_estabelecimento_saude)

# total de estabelecimentos na atenção primária
total_estab_pri <- df_limpo |> 
  filter(nivel_atencao == "atenção primária") |> 
  summarise(n = n_distinct(codigo_estabelecimento_saude)) |> 
  pull(n)
# total de estabelecimentos na atenção secundária
total_estab_sec <- df_limpo |> 
  filter(nivel_atencao == "atenção secundária") |> 
  summarise(n = n_distinct(codigo_estabelecimento_saude)) |> 
  pull(n)
# total de estabelecimentos na atenção terciária
total_estab_ter <- df_limpo |> 
  filter(nivel_atencao == "atenção terciária") |> 
  summarise(n = n_distinct(codigo_estabelecimento_saude)) |> 
  pull(n)
# Estabelecimentos nos municípios
total_estab_muni <- df_limpo |> 
  group_by(nome_do_municipio) |> 
  summarise(n = n_distinct(codigo_estabelecimento_saude)) |> 
  arrange(desc(n)) 
```

A 11º região de saúde do Ceará totaliza `r format(muni_pop, big.mark = ".", decimal.mark = ",")` mil habitantes o que representa `r format(rz_muni_macro, big.mark = ".", decimal.mark = ",")`% da população da Superintendência Regional de Saúde Norte (`r format(macro_norte_pop, big.mark = ".", decimal.mark = ",")` milhões) e `r format(rz_muni_ce, big.mark = ".", decimal.mark = ",")`% da população do Ceará (`r format(ce_pop, big.mark = ".", decimal.mark = ",")` milhões) e (IBGE, 2025). Foram identificados `r total_estab` estabelecimentos, sendo a maior parte pertencente à atenção primária (`r total_estab_pri`) e localizados no município de Sobral (ver tabela 1).

```{r}
#| echo: false
#| warning: false

# tabela com estabelecimentos na ADS sobral
tabela_estab <- df_limpo |> 
  group_by(nome_do_municipio, estabelecimento, nivel_atencao) |> 
  summarise(unidades_fisicas = n_distinct(codigo_estabelecimento_saude),
            total_vinculos = n (),
            .groups = "drop") |>
  ...
  set_header_labels(nome_do_municipio = "Município",
                    ...
                    unidades_fisicas = "Número de estabelecimentos",
                    total_vinculos = "Número de vínculos") |> 
  autofit() |> 
  set_caption("Distribuição dos estabelecimentos de saúde na ADS de Sobral")
...
Dos 171 registros de psicólogos, observou-se que havia um grupo de 21 profissionais com mais de um vínculo. A maioria (19) possuía dois registros, enquanto dois psicólogos apresentavam três vínculos ativos. Dos 21, 13 profissionais atuavam exclusivamente no nível primário (12 com dois vínculos e um com três). Quanto à atenção secundária, três profissionais mantinham dois vínculos cada. A análise de integração entre níveis revelou que três profissionais atuavam de forma híbrida entre as atenções primária e secundária (um vínculo em cada), enquanto um transitava entre a secundária e a terciária. Por fim, identificou-se um profissional com atuação mista, com dois vínculos na primária e um na secundária. 

Ainda assim, para a análise da rede assistencial, optou-se pela manutenção da totalidade dos vínculos. Tal decisão justifica-se pelo fato este estudo procurou investigar a ferta de psicólogos nos serviços em cada ponto da rede. Excluir vínculos sobrepostos (profissionais com acúmulo de cargos) resultaria em uma subestimação da cobertura assistencial em determinados níveis e uma superestimação em outros, uma vez que um único profissional pode desempenhar funções distintas e essenciais em diferentes estabelecimentos ou níveis de complexidade (primária, secundária ou terciária) (Constituição da República Federativa do Brasil, 1988, Art. 37, XVI). Assim, cada registro foi tratado como um posto de trabalho ativo e, assim, refletir a disponibilidade real de atendimento à população em cada serviço de saúde.
...
cor_plot <- df_plot |>
  ...
  geom_smooth(method = "lm", color = "black", se = F, linewidth = 1.1, n = 1000) +
  facet_wrap(~categoria, scales = "fixed") +
  scale_y_log10(breaks = c(1, 2, 5, 10, 20, 50)) +
  scale_x_log10(breaks = c(10000, 25000, 50000, 100000, 250000), 
